...
const Login = ({navigation}) => {
  ...
  const cbIGLogin = (data) => {
    // Attempt a login using the Facebook login dialog asking for default permissions.
    dispatch(authActions.login({email:data.user_id, password:data.user_id}));
  };
    ...
    <Screen footer={false} style={tw`items-center flex-1`} scrollEnabled={true}>
      <ViewY spacing={8} style={tw`items-stretch flex-1 pt-16`}>
        ...
        <ViewY spacing={16}>
          ...
          <ButtonYellow onPress={onLoginPress}>Ingresar</ButtonYellow>
        </ViewY>
        <ViewY spacing={4}>
          ...
          <View style={tw`items-center flex-row justify-center`}>
            ...
            <TouchableOpacity
              onPress={handlePlus}>
              <Text style={tw`text-orange-500 text-lg  py-1 px-2`}>registrate</Text>
            </TouchableOpacity>
          </View>
        </ViewY>
      </ViewY>
